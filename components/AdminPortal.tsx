...
const AdminPortal: React.FC<AdminPortalProps> = ({ onLogout }) => {
  ...
  return (
    <div className="h-screen overflow-hidden bg-gray-900 text-white flex font-sans">
      ...
      <div className="w-20 bg-black border-r border-gray-800 flex flex-col items-center py-6 z-20">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-blue-600 mb-8 shadow-lg shadow-emerald-500/20"></div>
        ...
        <div className="flex flex-col gap-6 w-full">
            ...
            >
                ...
                <span className="absolute left-16 bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap border border-gray-700">Interviews</span>
            </button>
            ...
            >
                ...
                <span className="absolute left-16 bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap border border-gray-700">Job Board</span>
            </button>
        </div>
      ...
      <div className="w-80 bg-gray-800/50 backdrop-blur-xl border-r border-gray-700 flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto">
            {view === 'interviews' ? (
                ...
                jobs.length === 0 ? (
                    <div className="p-6 text-gray-500 text-sm italic">No active jobs.</div>
                ) : (
                    jobs.map(job => (
                        <button 
                            key={job.id}
                            onClick={() => { setSelectedJobId(job.id); setIsCreatingJob(false); }}
                            className={`w-full text-left p-4 border-b border-gray-700/50 hover:bg-gray-700/50 transition-all ${selectedJobId === job.id && !isCreatingJob ? 'bg-gray-700 border-l-4 border-l-emerald-500 pl-3' : ''}`}
                        >
                            ...
                            <div className="mt-2 flex gap-2">
                                <span className="text-[10px] bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded-full">{job.type}</span>
                                <span className="text-[10px] bg-purple-500/20 text-purple-300 px-2 py-0.5 rounded-full">{job.department}</span>
                            </div>
                        </button>
                    ))
                )
      ...
      {/* 3. Content Area */}
      <div className="flex-1 bg-gradient-to-br from-gray-900 via-gray-900 to-[#111] overflow-y-auto p-8 relative">
        ...
        {view === 'interviews' && (
            selectedReport ? (
                <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
                    {/* Interview Report View (Same as before, styled slightly better) */}
                    <div className="flex justify-between items-start">
                        <div>
                            <h2 className="text-4xl font-bold text-white tracking-tight">{selectedReport.applicant.name}</h2>
                            <p className="text-xl text-emerald-400 mt-1">{selectedReport.applicant.role}</p>
                            <p className="text-sm text-gray-500">{selectedReport.applicant.email}</p>
                        </div>
                        <div className="text-center bg-gray-800/40 backdrop-blur border border-gray-700 p-4 rounded-2xl shadow-2xl">
            ...
            isCreatingJob ? (
                ...
                <div className="max-w-3xl mx-auto animate-fade-in">
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <button onClick={() => setIsCreatingJob(false)} className="text-gray-400 hover:text-white">Cancel</button>
                    </div>

                    <div className="bg-gray-800/40 border border-gray-700/50 rounded-2xl p-8 shadow-2xl backdrop-blur-sm">
                            ...
                            <div className="col-span-2">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button 
                                        ...
                                        className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white px-6 py-2 rounded-xl font-medium shadow-lg shadow-purple-500/30 flex items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        {isGenerating ? (
                                ...
                                <p className="text-xs text-gray-500 mt-2 ml-1">Tip: Enter a title and click Magic Generate to let AI write the description.</p>
                            </div>
                        ...
                        <div className="flex justify-end gap-4">
                            <button 
                                ...
                                className="px-8 py-3 bg-emerald-500 hover:bg-emerald-400 text-black font-bold rounded-xl shadow-lg shadow-emerald-500/20 transition-all transform hover:-translate-y-0.5"
                            >
                                Publish Job
                            </button>
                        </div>
                ...
                <div className="max-w-4xl mx-auto animate-fade-in">
                    <div className="flex justify-between items-start mb-8">
                        <div>
                            ...
                            <h2 className="text-4xl font-bold text-white mb-2">{selectedJob.title}</h2>
                    ...
                    <div className="grid grid-cols-3 gap-8">
                        ...
                        <div className="space-y-6">
                            <div className="bg-gradient-to-br from-emerald-900/40 to-gray-800/40 border border-emerald-500/20 rounded-2xl p-6">
                                <h3 className="text-sm font-bold text-emerald-400 uppercase tracking-widest mb-4">Quick Actions</h3>
                                <button className="w-full py-3 bg-white text-black font-bold rounded-lg mb-3 hover:bg-gray-200 transition-colors">
                                    View Applicants (0)
                                </button>
                                <button className="w-full py-3 bg-gray-700 text-white font-semibold rounded-lg hover:bg-gray-600 transition-colors">
                                    Edit Posting
                                </button>
                            </div>
                        </div>
