...
type AppStep = 'form' | 'landing' | 'call';
...
function App() {
    ...
    try {
        if ((window as any).aistudio) {
            ...
            if (!hasKey) {
                ...
                // Race condition mitigation: Assume success and proceed without checking return value
            }
        }
    } catch (e) {
        console.error("API Key flow error:", e);
        // We warn but try to proceed to permissions
    }
  ...
  if (step === 'form') {
    return <ApplicantForm onSubmit={handleFormSubmit} />;
  }

  if (step === 'call' && hasPermissions && applicantData) {
    return (
